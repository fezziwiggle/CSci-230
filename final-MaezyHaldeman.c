...
//CSCI 230: Final (modification of HW 9)
...
void BUBBLE_SORT(float array[], int count) { //from Marsh's solution
int i, j;
float swap;
...
float *INTARRAY(int size, FILE *(stream), float intArray[]) { //fills our array from integers from the data file //should it be float intArray?
...
void SORTARRAY(float intArray[], int start, int end, float balanced[]) { //sorts our array into a balanced array //should it be float intArray?
...
FILE *stream;
	...
	size = SCAN(&stream);
	intArray = (float *)malloc(size * sizeof(float));
	...
	balanced = malloc(size * sizeof(float));		
