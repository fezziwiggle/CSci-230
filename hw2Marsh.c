...
//struct data autos[100];
    ...
    while (1) {
       ...
       switch (option) {
          case '1': 
            size = scan();
            autos = (struct data *) malloc(size * sizeof(struct data));
            load(size, autos);
            sortf(size, autos);
            printhl(size, autos);
            free(autos);
